...
namespace StrorageRestApi.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<AuthenticationResult> RegisterAsync(string email, string password, 
            string name, string confirm, string phone)
        {
            ...
            if (existingUser != null)
            {
                return new AuthenticationResult
                {
                    Errors = new[] { "User with this email address is already exist" }
                };
            }
        ...
        public async Task<UserInfoResponse> GetInfoResponse(string token)
        {
            var userId = await _context.RefreshTokens.FirstOrDefaultAsync(x => x.Token == token);
            var user = await _userManager.FindByIdAsync(userId.UserId);

            UserInfoResponse thisUser = new UserInfoResponse
            {
                Id = user.Id,
                Name = user.Name,
                Email = user.Email,
